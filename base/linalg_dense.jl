...
function BunchKaufman{T<:BlasFloat}(A::Matrix{T}, uplo::Char)
    LD, ipiv = LAPACK.sytrf!(uplo , copy(A))
    BunchKaufman(LD, ipiv, uplo)
end
BunchKaufman{T<:Real}(A::StridedMatrix{T}, uplo::Char) = BunchKaufman(float64(A), uplo)
BunchKaufman(A::StridedMatrix) = BunchKaufman(A, 'U')

copy(B::BunchKaufman) = BunchKaufman(copy(B.LD), copy(B.ipiv), copy(B.uplo))

size(B::BunchKaufman) = size(B.LD)
size(B::BunchKaufman,d::Integer) = size(B.LD,d)
## need to work out how to extract the factors.
#factors(B::BunchKaufman) = LAPACK.syconv!(B.uplo, copy(B.LD), B.ipiv)
function ref{T}(B::BunchKaufman{T}, d::Symbol)
    if d == :D
        n = size(B, 1)
        D = zeros(T, n, n)
        i = 1
    ...
    if d == symbol(B.uplo)
        if d == :U
            ...
            while i > 0
                PUk = eye(T, n)
                if B.ipiv[i] > 0
                    PUk[1:i-1,i] = B.LD[1:i-1,i]
                    if i != B.ipiv[i]
                        PUk[[i,B.ipiv[i]],:] = PUk[[B.ipiv[i],i],:]
                    end
                    i -= 1
                else
                    PUk[1:i-2,i-1:i] = B.LD[1:i-2,i-1:i]
                    if i-1 != -B.ipiv[i]
                        PUk[[i-1,-B.ipiv[i]],:] = PUk[[-B.ipiv[i],i-1],:]
                    end
                    i -= 2
                end
                U *= PUk
            ...
            while i <= n
                PLk = eye(T, n)
                if B.ipiv[i] > 0
                    PLk[i+1:n,i] = B.LD[i+1:n,i]
                    PLk[[i,B.ipiv[i]], :] = PLk[[B.ipiv[i],i], :]
                    PLk[:, [i,B.ipiv[i]]] = PLk[:, [B.ipiv[i],i]]
                    i += 1
                else
                    PLk[i+2:n,i:i+1] = B.LD[i+2:n,i:i+1]
                    PLk[[i+1,-B.ipiv[i]], :] = PLk[[-B.ipiv[i],i+1], :]
                    PLk[:, [i+1,-B.ipiv[i]]] = PLk[:, [-B.ipiv[i],i+1]]
                    i += 2
                end
                L *= PLk
...
function ref(C::CholeskyDense, d::Symbol)
    if d == :UL || symbol(C.uplo) == d
        return C.UL
    else
        error("No such factor")
    end
end
function ref(C::CholeskyDense, d::Integer)
    if d == 1 return C.UL end
    error("No such factor")
end
...
function ref{T<:BlasFloat}(C::CholeskyPivotedDense{T}, d::Symbol)
    if d == :UL || symbol(C.uplo) == d return C.UL end
...
function ref(C::CholeskyPivotedDense, d::Integer)
    if d == 1 return C[:UL] end
    if d == 2 return C[:p] end
    error("No such property")
end
...
function ref(A::LUDense, d::Integer)
    if d == 1 return A[:L] end
    if d == 2 return A[:U] end
    if d == 3 return A[:p] end
    error("No such property")
end
...
function sqrtm(A::Matrix, cond::Bool)
    if ishermitian(A) return sqrtm(Hermitian(A), cond) end
    m, n = size(A)
    if m != n throw(LAPACK.DimensionMismatch("Matrix must be square")) end
...
sqrtm(A::AbstractMatrix) = sqrtm(A, false)
...
eigvals(m::SymTridiagonal, vl::Real, vu::Real) = LAPACK.stebz!('V', 'E', vl, vu, 0, 0, -1.0, copy(m.dv), copy(m.ev))[1]
...
Triangular(A::Matrix, uplo::Char) = Triangular(A, uplo, all(diag(A) .== 1) ? 'U' : 'N')
...
function inv(A::Triangular)
    r, info = LAPACK.trtri!(A.uplo, A.unitdiag, copy(A.UL))
    if info > 0 throw(LAPACK.SingularException(info)) end
    return r
end
...
copy(A::Hermitian) = Hermitian(A.S, A.uplo)
